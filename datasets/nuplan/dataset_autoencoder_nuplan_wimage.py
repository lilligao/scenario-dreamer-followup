...
from utils.cam_img_utils import trans_matrix_inv,trans_matrix, transform, project_cam_to_image, project_cam_to_image_nodrop, plot_projection, plot_projection_all_views, plot_topdown_lanes_and_agents, load_cam_views, create_se2_4x4_matrix, get_3d_box_corners
...
class NuplanDatasetAutoEncoder(Dataset):
    ...
    def get_data(self, data, idx):
        ...
        # ───────────────────────────────────────────────────────────────
            ...
            cam_img_stack, T_cam_tf_stack, T_cam_ego_stack, intrinsics_stack, width, height  = load_cam_views(
                cam_infos=cam_infos,
            ...
            #T_global2ego = trans_matrix_inv(ego_state_og[1], ego_state_og[0])
            #agent_ego = transform(agent_global, T_global2ego)
            if self.cfg.debug_vis:
                ...
                for i in range(len(self.cam_order)):
                    agent_ego = transform(agent_global, T_cam_ego_stack[i])
                    agent_cam = transform(agent_ego, T_cam_tf_stack[i])
                    agent_uv, agent_mask = project_cam_to_image(agent_cam, intrinsics_stack[i], (width, height)) # 
                ...
                for i in range(len(self.cam_order)):
                    corners_ego = transform(corners_global.reshape(-1, 3), T_cam_ego_stack[i]).reshape(-1, 8, 3)
                    corners_cam = transform(corners_ego.reshape(-1, 3), T_cam_tf_stack[i]).reshape(-1, 8, 3)
            ...
            for lg_type in lg_dict.keys():
                ...
                if self.cfg.debug_vis:
                    ...
                    for i in range(len(self.cam_order)):
                        road_ego = transform(road_global, T_cam_ego_stack[i])  # shape (num_lanes * lane_length, 3)
                        road_cam = transform(road_ego, T_cam_tf_stack[i])
                        road_uv, road_mask = project_cam_to_image(road_cam, intrinsics_stack[i], (width, height)) 
                    ...
                    #     file_prefix='{}_scene_{}_lg_{}'.format(raw_file_name, idx,  0 if lg_type == 'regular' else 1)
                    # )
