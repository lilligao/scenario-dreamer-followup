...
from utils.cam_img_utils import project_cam_to_image, plot_projection, plot_projection_all_views, plot_topdown_lanes_and_agents, load_cam_views, transform

class NuplanDatasetAutoEncoder3D(Dataset):
    ...
    def __init__(self, cfg: Any, split_name: str = "train", mode: str = "train") -> None:
        ...
        super(NuplanDatasetAutoEncoder3D, self).__init__()
        self.cfg = cfg
    ...
    def get_data(self, data, idx):
        ...
        # ───────────────────────────────────────────────────────────────
            ...
            if self.load_images:
                ...
                cam_img_stack, T_ego_cam_stack, intrinsics_stack, width, height  = load_cam_views(
                    cam_infos=cam_infos,
            ...
            if self.load_images and self.cfg.debug_vis:
                ...
                for i in range(len(self.cam_order)):
                    agent_cam = transform(agent_3d, T_ego_cam_stack[i])
            ...
            for lg_type in lg_dict.keys():
                ...
                road_flat = road_points_3d.reshape(-1, 3)  # shape (num_lanes * lane_length, 3)
                ...
                if self.load_images and self.cfg.debug_vis:
                    ...
                    road_uv_list = []
                    for i in range(len(self.cam_order)):
                        road_cam = transform(road_flat, T_ego_cam_stack[i])
...
def main(cfg):
    ...
    dset = NuplanDatasetAutoEncoder3D(cfg.ae.dataset, split_name='train')
    print(len(dset))
    np.random.seed(10)
    random.seed(10)
